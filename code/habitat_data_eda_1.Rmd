...
knitr::opts_knit$set(root.dir = "C:/Users/Becca/Documents/Grad_School/Research/Projects/Sound_Habitat/data/")
getwd()
```
...
data = read.csv("sound_habitat_data_2020.csv", header = TRUE)

## filter to data needed for mapping
#subset = data[,c(1,2,6,7,8,10,24:27)]
#subset = unique(subset)

#write.csv(subset,file = "sound_hab_data_map_subset.csv")

```

## Summarize each transect (not used)

```{r}
# transect_means = data %>%
#   group_by(deploy_num,site,transect) %>%
#   summarise(bare = mean(bare), halodule = mean(halodule), zostera = mean(zostera),
#             ruppia= mean(ruppia), algae = mean(algae), oyster = mean(oyster), spartina = mean(spartina),
#             juncus = mean(juncus), other = mean(other))
# 
# percent_cover_mean = data.frame(gather(transect_means, "identity", "perc_cover", -site, - transect, -deploy_num))
# percent_cover_mean$identity = factor(percent_cover_mean$identity)
# 
# ggplot(percent_cover_mean, aes(x = perc_cover, y = identity))+ theme_bw()+
#   geom_point()
...
# Summarize cover by taxa, with seagrass spp combined
...
siteMean_pc_comb$identity = factor(siteMean_pc_comb$identity, levels = c("bare","seagrass","oyster","saltmarsh","algae","other"))

#levels(siteMean_pc_comb$identity) = c("bare","seagrass","oyster","saltmarsh","algae","other")
...
ggplot(total_cover , aes(x = reorder(site, total_cover), y = total_cover, fill = Habitat, color = Habitat))+theme_bw()+
  geom_col()+ 
...
```{r}
seagrass = read.csv("seagrass_all.csv", header = TRUE)
seagrass$id = paste(seagrass$site, seagrass$Transect, sep = "_")
seagrass_max = read.csv("seagrass_max_canopy.csv", header = TRUE) #max canopy as calculated in excel
...
# Calculates site mean by averageing the max of each transect --> this is not how the cover is calculated
# Do I ned to go back and summarize the max canopy by site more generally?
...
```{r}
trawl = read.csv("summer_trawl_data_2019.csv", header = TRUE)
...
species_list = read.csv("species_list.csv", header = TRUE)
...
# make a unique id for each site and tow to pair with the soniferous
rep_metrics$uniqID = mutate(paste(rep_metrics$Project.Site.Name, rep_metrics$Tow_number, sep = "_"))
soniferous$uniqID = paste(soniferous$Project.Site.Name, soniferous$Tow_number, sep = "_")

comm_metrics = left_join(rep_metrics[,c(1:12)], soniferous[,4], by = "Project.Site.Name") # why wont this work!!

# write.csv(rep_metrics, "rep_metrics.csv")
## Manually convertered trawl replicates into rows

rep_metrics = read.csv("rep_metrics.csv", header = T)
...
std_catch_sum = data.frame(pivot_wider(test_catch,id_cols = c(1,3,7:9), names_from = "identity",values_from = "std_abund_sum"))

# Community metrics summed
trawl_metrics  = std_catch_sum[,c(1:6)]
trawl_metrics$total_abund_sum = rowSums(std_catch_sum[,7:42])
trawl_metrics$comm_richness_sum = apply(std_catch_sum[,c(7:42)]>0, 1, sum)
trawl_metrics$comm_shannonD_sum = diversity(std_catch_sum[,7:42], "shannon")
...
spl_pmHT = as.data.frame(read.csv("avg_pmHT_spl.csv", header = TRUE))
...
ecometrics$site_code = c("C7","MF2","SG10","SG12","C2","MF1","SG2","SG4","CB","MA","SB","SC","C8","MF7","SG6","SG9",
...
write.csv(ecometrics, "ecometrics.csv")
...
ggplot(aes(x = seagrass_comb, y = avg_spl_low), data = hab_sound_data)+
  geom_point()

ggplot(aes(x = richness_comb, y = avg_spl_low), data = hab_sound_data)+
  geom_point()
cor(hab_sound_data$richness_comb,hab_sound_data$avg_spl_low)

ggplot(aes(x = saltmarsh, y = avg_spl_low), data = hab_sound_data)+
  geom_point()

ggplot(aes(x = oyster, y = avg_spl_high), data = hab_sound_data)+
  geom_point()
```
