...
## Vegan NMDS trial - pmHT site average

```{r}
# go back to matlab and calculate single average for each site - so ord has 24 points, then pair that with the env data 
...
env_cor = cor(env_data[,c(4:23)])
env_filt = env_data %>%
  select(-index, -deploy_num.x, -site, -bare)
...
ggplot(aes(x = NMDS1, y = NMDS2), data = nmdsScores)+ theme_bw()+
  ...
  geom_text(data=envScores,aes(x=NMDS1,y=NMDS2,label=rownames(envScores)),size=5)+
  geom_text(aes(label = site), data = nmdsScores)
